...
trait TestSpecWithoutSystem extends FlatSpec with Matchers with ScalaFutures with TryValues with OptionValues with BeforeAndAfterAll {
  implicit val timeout: Timeout = Timeout(10.seconds)
  implicit def system: ActorSystem
  implicit def ec: ExecutionContext = system.dispatcher
  implicit def mat: Materializer = ActorMaterializer()
  implicit def log: LoggingAdapter = Logging(system, this.getClass)
